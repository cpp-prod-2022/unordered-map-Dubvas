...
private:
	...
	private:
		...
		iterator insertNode(iterator it, Node* node) {
	...
	struct ListElement {
		NodeType value;
		size_t hash;
		ListElement(NodeType&& v, size_t h) : value(NodeType{ std::move(const_cast<Key&>(v.first)), std::move(v.second) }), hash(h) {};
	...
	constexpr static size_t BASE_BUCKETS = 4;
	...
	std::pair<LIterator, bool> myFind(const Key& k) const {
		size_t h = hasher(k);
		auto start_o = table[h % table.size()];
		if (!start_o)
	...
	LIterator inPlaceInsert(LIterator it, NodeType&& ins) {
		size_t h = hasher(ins.first);
		it = bucket.insert(it, ListElement(std::move(ins), h));
...
public:
	...
	Value& operator[](const Key& k) {
		auto [it, res] = myFind(k);
		if (!res)
			it = inPlaceInsert(it, NodeType{ k, Value() });
		return it->value.second;
	}
	...
	Value& operator[](Key&& k_rv) {
		...
		auto [it, res] = myFind(k);
		if (!res)
			it = inPlaceInsert(it, NodeType{ std::move(k), Value() });
		return it->value.second;
	}
	...
	iterator erase(const_iterator pos) {
		auto lit = pos.it;
		auto h = lit->hash;
		if (*table[h % table.size()] == lit) {
			auto nlit = lit;
			++nlit;
			if (nlit->hash % table.size() != h % table.size())
				table[h % table.size()] = std::nullopt;
			else
				table[h % table.size()] = nlit;
		}
